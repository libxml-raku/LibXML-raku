...
struct xmlEntitiesTable is repr('CPointer') {
    ...
    method xmlFreeEntitiesTable( --> void) is native(LIB) {*};
}
    sub xmlCleanupPredefinedEntities( --> void) is native(LIB) {*};
    sub xmlEncodeSpecialChars(const xmlDoc * $doc, xmlCharP $input --> xmlCharP) is native(LIB) {*};
    sub xmlInitializePredefinedEntities( --> void) is native(LIB) {*};
