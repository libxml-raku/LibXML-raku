...
struct xmlValidCtxt is repr('CStruct') {
    ...
    has unsigned int $.finishDtd; # finished validating the Dtd ?
    ...
    method xmlFreeValidCtxt( --> void) is native(LIB) {*};
...
struct xmlValidState is repr('CPointer') {
    ...
    sub xmlSnprintfElementContent(Str $buf, int32 $size, xmlElementContent $content, int32 $englob --> void) is native(LIB) {*};
    sub xmlSprintfElementContent(Str $buf, xmlElementContent $content, int32 $englob --> void) is native(LIB) {*};
    sub xmlValidGetPotentialChildren(xmlElementContent * $ctree, const xmlChar ** $names, int * $len, int32 $max --> int32) is native(LIB) {*};
