...
also does LibXML::_Rawish[xmlDoc, <encoding setCompression getCompression standalone URI wellFormed set-flags>];
...
=para After running this function, the LibXML::Element elementIndex() method returns the
    index of the element, with the root element having an index of 1.

=para If this function has not been run, elementIndex() returns 0 for all elements.
